Shader "LFO/Volumetric (Opaque)"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            const float k2 = 2;
            const float n0 = 1;
            #define MAX_ITP_LOOPS = 6

            float ITP(in float3 rayOrigin,in float3 rayDirection){
                float a = minmax.x;
                float b = minmax.y;
                float ya = map(rayOrigin + rayDirection * a);
                float yb = map(rayOrigin + rayDirection * b);
                int i = 0;

                float nhalf = log2((b-a)/(2 * e));
                float nmax = nhalf + n0;
                ...
                while((b-a) > (2*e))
                {
                    //Pre calcs
                    float xhalf = (a+b)/2;
                    float r = (e* pow(2, nmax - i)) - ((b-a)/2);
                    float sigma = k1 * pow((b-a), k2); 


                    //Interpolate
                    float xf = ((yb* a) - (ya * b))/(yb-ya);

                    //truncate
                    float xt = 0;
                    float sgn = sign(xhalf-xf);
                    if(sigma <= abs(xhalf - xf)) xt = xhalf + sgn * sigma; else xt = xhalf;

                    //Project
                    float xitp = 0;
                    if(abs(xt-xhalf) <= r) xitp = xt; else xitp = xhalf - sgn * r;

                    //Updating intervals
                    float yitp = map(rayOrigin + rayDirection * xitp);
                    if(yitp > 0)
                        {b = xitp; yb = yitp;}
                    else if(yitp < 0)
                        {a = xitp; ya = yitp;} 
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                if(!LogBisect(orw(rayPos, 0), orw(rayDir,0), dstToBox, dstToBoxFar))
                    discard;
                #else
                float itp = ITP(orw(rayPos, 0), orw(rayDir,0));
                float3 rootPos = orw(rayPos, 0) + orw(rayDir,0) * itp;
                return float4(map(rootPos),0,0,1);
                #endif
